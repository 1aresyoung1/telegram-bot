import os
import logging
from telegram import Update, ReplyKeyboardMarkup
from telegram.ext import (
    Application,
    CommandHandler,
    MessageHandler,
    ContextTypes,
    filters,
)

# ================== НАЛАШТУВАННЯ ==================
TOKEN = os.getenv(8596592294:AAHvoj-GVwfToT103XWOcvMMUoqE2DrkflU)

logging.basicConfig(
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    level=logging.INFO,
)

# ================== СТАНИ КОРИСТУВАЧА ==================
MODE_NONE = "none"
MODE_HELP = "help"
MODE_SUGGEST = "suggest"

user_modes = {}

# ================== КНОПКИ ==================
MAIN_KEYBOARD = ReplyKeyboardMarkup(
    [
        ["🔐 Перевірити пароль", "🔗 Перевірити посилання"],
        ["🎲 Згенерувати пароль", "🛡 Поради з безпеки"],
        ["ℹ️ Про бота", "🆘 Допомога"],
        ["💡 Пропозиції для оновлення"],
    ],
    resize_keyboard=True,
)

# ================== START ==================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_modes[update.effective_user.id] = MODE_NONE
    await update.message.reply_text(
        "👋 Привіт!\n\n"
        "Я бот з кібербезпеки 🔐\n"
        "Допоможу перевірити паролі, посилання та відповісти на проблеми.\n\n"
        "⬇️ Обери дію з меню:",
        reply_markup=MAIN_KEYBOARD,
    )

# ================== ОБРОБКА КНОПОК ==================
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    user_id = update.effective_user.id

    # ---------- ПЕРЕВІРКА ПАРОЛЯ ----------
    if text == "🔐 Перевірити пароль":
        user_modes[user_id] = MODE_NONE
        await update.message.reply_text(
            "🔐 Надішли пароль.\n"
            "Я оціним його надійність."
        )
        return

    # ---------- ПЕРЕВІРКА ПОСИЛАННЯ ----------
    if text == "🔗 Перевірити посилання":
        user_modes[user_id] = MODE_NONE
        await update.message.reply_text(
            "🔗 Надішли посилання.\n"
            "Я перевірю, чи воно може бути фішинговим."
        )
        return

    # ---------- ГЕНЕРАЦІЯ ПАРОЛЯ ----------
    if text == "🎲 Згенерувати пароль":
        user_modes[user_id] = MODE_NONE
        await update.message.reply_text(
            "🎲 Надійний пароль:\n\n"
            "`Q9!f@A7Lm#2X`",
            parse_mode="Markdown",
        )
        return

    # ---------- ПОРАДИ ----------
    if text == "🛡 Поради з безпеки":
        user_modes[user_id] = MODE_NONE
        await update.message.reply_text(
            "🛡 Основні поради:\n\n"
            "• Не переходь за підозрілими посиланнями\n"
            "• Не вводь коди з SMS\n"
            "• Використовуй різні паролі\n"
            "• Увімкни 2FA"
        )
        return

    # ---------- ПРО БОТА ----------
    if text == "ℹ️ Про бота":
        user_modes[user_id] = MODE_NONE
        await update.message.reply_text(
            "ℹ️ Цей бот створений для допомоги з кібербезпеки.\n\n"
            "Він не зберігає твої дані та не передає інформацію третім особам."
        )
        return

    # ---------- ДОПОМОГА ----------
    if text == "🆘 Допомога":
        user_modes[user_id] = MODE_HELP
        await update.message.reply_text(
            "🆘 Опиши свою проблему **одним повідомленням**.\n"
            "Я спробую допомогти 🤖"
        )
        return

    # ---------- ПРОПОЗИЦІЇ ----------
    if text == "💡 Пропозиції для оновлення":
        user_modes[user_id] = MODE_SUGGEST
        await update.message.reply_text(
            "💡 Напиши свою пропозицію або ідею для покращення бота.\n"
            "Вона буде врахована 🚀"
        )
        return

    # ---------- ОБРОБКА ТЕКСТУ ----------
    mode = user_modes.get(user_id, MODE_NONE)

    if mode == MODE_HELP:
        user_modes[user_id] = MODE_NONE
        await update.message.reply_text(ai_help_answer(text))
        return

    if mode == MODE_SUGGEST:
        user_modes[user_id] = MODE_NONE
        await update.message.reply_text(
            "✅ Дякую!\n"
            "Твоя пропозиція збережена і буде розглянута 🙌"
        )
        return

    # ---------- ЗАХИСТ ВІД СМІТТЯ ----------
    await update.message.reply_text(
        "ℹ️ Будь ласка, користуйся кнопками меню ⬇️",
        reply_markup=MAIN_KEYBOARD,
    )

# ================== ЛОКАЛЬНИЙ ШІ ==================
def ai_help_answer(problem: str) -> str:
    p = problem.lower()

    if any(word in p for word in ["злам", "вкрали", "хак"]):
        return (
            "🚨 Можливий злам!\n\n"
            "Що робити:\n"
            "• Терміново змінити паролі\n"
            "• Вийти з усіх сесій\n"
            "• Увімкнути 2FA\n"
            "• Перевірити пристрій антивірусом"
        )

    if any(word in p for word in ["посилання", "сайт", "фішинг"]):
        return (
            "🔗 Схоже на фішинг.\n\n"
            "Рекомендації:\n"
            "• Не вводь дані\n"
            "• Перевір домен\n"
            "• Ігноруй повідомлення"
        )

    if "пароль" in p:
        return (
            "🔐 Рекомендації для пароля:\n\n"
            "• 12–16 символів\n"
            "• Великі й малі літери\n"
            "• Цифри та спецсимволи\n"
            "• Унікальний для кожного сайту"
        )

    return (
        "🤖 Я проаналізував повідомлення.\n\n"
        "Рекомендую не передавати особисті дані та діяти обережно.\n"
        "За потреби — уточни деталі."
    )

# ================== ЗАПУСК ==================
def main():
    app = Application.builder().token(TOKEN).build()

    app.add_handler(CommandHandler("start", start))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_menu))

    app.run_polling()

if __name__ == "__main__":
    main()
